...
em <- emtrends(m1.int, "Microsite.x", var = "ndvi")
pairs(em)
summary(em)
test(em)
plot(em)
...
ggplot(plt.ag, aes(Microsite.x, rich)) + geom_boxplot()


# native richness only
...
summary(m2.int)
m2.int3 <- glmmTMB(abun ~  arid * Microsite.x + ndvi + (1|Site), family = "nbinom2", plt.ag) 
...
prov.abun <- pivot_longer(plt.ag, cols = invasive:native, names_to = "prov", values_to = "abun.prov")


m2.int <- glmmTMB(abun.prov ~  arid * Microsite.x * prov  + (1|Site), family = "nbinom1", prov.abun)
m2.int2 <- glmmTMB(abun.prov ~  arid * Microsite.x * prov+ ndvi  + (1|Site), family = "nbinom2", prov.abun)

AIC(m2.int, m2.int2)


em <- emtrends(m2.int, ~Microsite.x*prov, var = "arid")
pairs(em)
test(em)
summary(m2.int)


em <- emtrends(m2.int, "prov", var = "arid")
pairs(em)
summary(em)
test(em)
plot(em)
...
ggplot(plt.ag, aes(arid, invasive, fill = Microsite.x)) + geom_smooth(method = "lm")


AIC(m2.int, m2.int2)


m2 <- glmmTMB(invasive ~  arid + Microsite.x + ndvi + (1|Site), family = "nbinom1", plt.ag)
AIC(m2, m2.int2)


m2.int3 <- glmmTMB(invasive ~  arid * Microsite.x + (1|Site), family = "nbinom1", plt.ag)
summary(m2.int3)
AIC(m2.int2, m2.int3)
...
```{r}
m2.int2 <- glmmTMB(native ~  arid * Microsite.x +(1|Site), family = "nbinom1", plt.ag)

summary(m2.int2)

ggplot(plt.ag, aes(arid, native, fill = Microsite.x)) + geom_smooth(method = "lm")

glmm.hp(m2.int2)


ggplot(plt.ag, aes(arid, native)) + geom_smooth(method = "lm") + geom_smooth(aes(arid, invasive), method = lm, color = "red") + facet_grid(~Microsite.x)
...
m7 <- glmmTMB(log(nat.cwm) ~ Microsite.x  + arid + gr.webers + (1|Site), plt.ag)
summary(m7)
car::Anova(m7)
...
#invasives
m8 <- glmmTMB(log(cwm.inv) ~ arid + Microsite.x  + gr.webers + (1|Site), plt.ag)
...
m2 <- glmmTMB(gr.richness ~ Microsite +arid + (1|Site), family = "poisson", ptsreps)

m3 <- glmmTMB(gr.richness ~ ndvi + Microsite +arid + (1|Site), family = "poisson", ptsreps)

m4 <- glmmTMB(gr.richness ~ ndvi + Microsite +(1|Site), family = "poisson", ptsreps)
...
m2 <- glmmTMB(gr.richness ~ ndvi +Microsite  * arid + (1|Site), family = "poisson", ptsreps)
summary(m2)

m3 <- glmmTMB(gr.richness ~ ndvi *Microsite + (1|Site), family = "poisson", ptsreps)
summary(m3)
...
m3.nb <- glmmTMB(gr.abun ~  arid + Microsite  + ndvi +(1|Site), family = "nbinom2", ptsreps)

m3.nb1 <- glmmTMB(gr.abun ~ arid + Microsite  + ndvi +(1|Site) + (1|year), family = "nbinom2", ptsreps)
...
m4.nb2 <- glmmTMB(gr.abun ~ arid + Microsite  + ndvi +(1|Site), family = "nbinom2", ptsreps)

AIC(m4.nb1, m4.nb2)



m4.int <- glmmTMB(gr.abun ~ arid * Microsite  + ndvi +(1|Site), family = "nbinom1", ptsreps)
summary(m4.int)

m4.int2 <- glmmTMB(gr.abun ~ arid + Microsite  * ndvi +(1|Site), family = "nbinom1", ptsreps)
summary(m4.int2)
...
m6 <- glmmTMB(percent ~ Size + Microsite + arid + mean.inv + mean.native  + mn.gr.abun+ (1|Year) + (1|Site), seeds)

summary(m6)
m6 <- glmmTMB(percent ~ Size + Microsite + ndvi  + mn.gr.abun+ gr.webers + arid + (1|Year) + (1|Site), seeds)
...
m11.9 <- glmmTMB(percent ~  ndvi + Microsite + mn.gr.abun *Size + arid + (1|Year)+ (1|Site), seeds)
summary(m11.9)
car::Anova(m11.9)
...
ggplot(seeds, aes(ndvi, percent, fill = Size)) + geom_smooth(method = "lm")
